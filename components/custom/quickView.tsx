    ...
    <div className="bg-white dark:bg-black py-16 px-6">
      <div className="max-w-350 mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            {item.images?.map((img: { id: string, url: string }) => (
              <button
                ...
                className={`relative w-20 h-20 flex-shrink-0 border-2 rounded-md overflow-hidden ${selectedImage === img.url ? "border-black" : "border-transparent"
                  }`}
              >
                <Image
                  src={img.url}
        ...
        <div className="flex flex-col gap-8">
          <div>
            <h1 className="text-4xl font-source-serif font-semibold font-bold mb-4">{item.title}</h1>
